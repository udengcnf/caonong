...
class MongoUtils(object):
    ...
    def update_single(cls, table = None, condition = None, model_dic = None):
        ...
        try:
            ...
            result = collection.find_one(condition, update_param_dic)
        except:
    ...
    @classmethod
    def update(cls, table = None, condition = None, model_dic = None, new = True): ## 默认不存在则创建
        result = None
        try:
            collection = cls.mongo_client()[table]
            update_param_dic = {'$set':model_dic}
            result = collection.update(condition, update_param_dic, upsert=new)
        except:
...
if __name__ == '__main__':
    ...
    # MongoUtils.insert_one(table='spider.test', model_dic={'a':'bcd'})
    # MongoUtils.insert_many(table='spider.test', model_dic_list=[{'11':'11', '22':'22'}, {'33':'33', '44':'44'}])
    #  MongoUtils.replace(table='spider', old_model_dic={'00':'7'}, new_model_dic={'2':'4'})
    # MongoUtils.delete(table='spider', model_dic={'a':'bcd'})
    MongoUtils.update(table='spider.test', condition = {'11':'101'}, model_dic = {'11':'100', 'a':'b', '22':'111'})
